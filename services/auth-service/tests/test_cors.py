...
CORSMiddleware wraps the whole ASGI app and adds its response header
regardless of the route/status code underneath - confirmed directly
(even a 404 gets the header) - so these tests use a dummy path rather
than needing a real, valid route per service.
"""


def test_cors_allows_the_configured_origin(client):
    response = client.get("/cors-test-path", headers={"Origin": "http://localhost:3000"})
...
def test_cors_omits_the_header_for_an_unlisted_origin(client):
    ...
    response = client.get("/cors-test-path", headers={"Origin": "http://evil.example.com"})
