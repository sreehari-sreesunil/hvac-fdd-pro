...
def _create_asset_type(client, auth_headers, name="RTU", metrics=None):
    response = client.post(
        "/asset-types",
        json={"name": name, "metrics": metrics or []},
...
def test_add_metric_definition_to_an_existing_asset_type(client, auth_headers):
    ...
    get_resp = client.get("/asset-types", headers=auth_headers)
...
def test_add_metric_definition_to_nonexistent_asset_type_returns_404(client, auth_headers):
    response = client.post(
...
def test_add_duplicate_metric_name_to_same_asset_type_returns_409(client, auth_headers):
...
def test_same_metric_name_is_fine_on_different_asset_types(client, auth_headers):
...
def test_add_metric_definition_requires_auth(client, auth_headers):
    asset_type = _create_asset_type(client, auth_headers, name="RTU")
